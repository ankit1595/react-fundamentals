...
## Learning:
  ...
  React is JavaScript frontend framework which uses same DOM APIs under the hood.

  Imperative vs Declarative

  Imperative is more of how to do and Declarative is what to do.

  For eg: there are some functions in JavaScript which are declarative in nature like Array.sort(), Array.forEach()

  But if we have to write in some other language like C, we have to describe each and every step how to sort an array. so if we write function each time we have to use it...it is an imperative way

  From above example we can infer 2 more things...

  one is declarative is same as functional programming(functions that can be reused)

  second: using declarative or functional programming approach leads to more clean and readable code.
